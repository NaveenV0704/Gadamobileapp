...
import { listMyGroupInvites, acceptGroupInvite, declineGroupInvite } from "../../services/groupsService";
...
export default function GroupsInvites() {
  ...
  const fetchInvites = async () => {
    ...
    try {
      const data = await listMyGroupInvites(headersRef.current);
      setItems(data);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const handleAccept = async (groupName: string, inviteId: number) => {
    setBusyId(inviteId);
    try {
      await acceptGroupInvite(groupName, inviteId, headersRef.current);
  ...
  const handleDecline = async (groupName: string, inviteId: number) => {
    setBusyId(inviteId);
    try {
      await declineGroupInvite(groupName, inviteId, headersRef.current);
      fetchInvites();
    } catch (error: any) {
  ...
  const renderInviteItem = ({ item }: { item: any }) => (
    <View className="bg-white p-4 mb-2 flex-row items-center border-b border-gray-100">
      <TouchableOpacity
        ...
        className="h-12 w-12 rounded-xl bg-gray-200 overflow-hidden"
      >
        {item.group_picture && (
          <Image
      ...
      <View className="ml-3 flex-1">
        <TouchableOpacity onPress={() => router.push(`/groups/${item.group_name}`)}>
          <Text className="font-bold text-gray-900 text-base" numberOfLines={1}>
            {item.group_title}
          </Text>
        </TouchableOpacity>
        <Text className="text-gray-500 text-xs" numberOfLines={1}>
          @{item.group_name} • invited by {item.fromUsername}
        </Text>
      </View>

      <View className="flex-row items-center ml-2">
        <TouchableOpacity
          onPress={() => handleAccept(item.group_name, item.inviteId)}
        ...
        <TouchableOpacity
          onPress={() => handleDecline(item.group_name, item.inviteId)}
          disabled={busyId === item.inviteId}
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      <View className="px-4 py-3 bg-white border-b border-gray-100 flex-row items-center">
        ...
        <Text className="text-xl font-bold text-gray-900">Group Invites</Text>
      </View>
      ...
      <FlatList
          ...
          !loading ? (
            <View className="items-center justify-center py-20 px-10">
              ...
              <Text className="text-gray-400 mt-4 text-center font-medium">
                No pending group invites at the moment.
              </Text>
            </View>
          ) : null
        }
        ...
        ListFooterComponent={
          loading ? <ActivityIndicator size="small" color="#2563EB" className="py-10" /> : null
        }
      />
    </SafeAreaView>
  );
