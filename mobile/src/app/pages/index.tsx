    ...
    >
      <View className="h-24 bg-gray-200">
        {item.page_cover && (
          <Image
            source={{
              uri: `${ASSET_BASE_URL}/${item.page_cover.replace(/^\/+/, "")}`,
            }}
      ...
      <View className="p-3 flex-row items-center">
        <View className="h-12 w-12 rounded-full bg-gray-300 border-2 border-white -mt-8 overflow-hidden">
          {item.page_picture && (
            <Image
              source={{
        ...
        <View className="ml-3 flex-1">
          ...
          <Text className="text-gray-500 text-xs">@{item.page_name}</Text>
        </View>
        <View className="items-end">
          <Text className="text-blue-600 font-semibold text-xs">
            {item.page_likes} likes
          </Text>
        </View>
      </View>
    </TouchableOpacity>
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      <View className="px-4 py-3 bg-white border-b border-gray-100 flex-row items-center justify-between">
        <View>
          ...
          <Text className="text-xs text-gray-500">
            Discover and manage pages
          </Text>
        </View>
        <View className="flex-row items-center">
          <TouchableOpacity
            onPress={() => router.push("/pages/invites")}
      ...
      <View className="p-4">
        ...
        {showFilters && (
          <View className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm mb-3">
            ...
            >
              ...
              >
                <Text
                  className={`text-xs ${categoryId === null ? "text-white" : "text-gray-600"}`}
                >
                  All
                </Text>
              </TouchableOpacity>
        ...
        <FlatList
          ...
          keyExtractor={(item) => String(item.page_id || item.pageId)}
